 ...
 * https://eips.BNBereum.org/EIPS/eip-165[EIP].
 *
...
interface IERC165 {
     ...
     * https://eips.BNBereum.org/EIPS/eip-165#how-interfaces-are-identified[EIP section]
...
interface IERC721 is IERC165 {
     ...
     *
     * WARNING: Usage of this mBNBod is discouraged, use {safeTransferFrom} whenever possible.
     *
     * Requirements:
     *
     * - `from` cannot be the zero address.
 ...
 * @dev See https://eips.BNBereum.org/EIPS/eip-721
 */
...
abstract contract ERC165 is IERC165 {
    ...
    /**
     * @dev Mapping of interface ids to whBNBer or not it's supported.
     */
...
library Strings {
    ...
    function toString(uint256 value) internal pure returns (string memory) {
        ...
        // https://github.com/oraclize/BNBereum-api/blob/b42146b063c7d6ee1358846c198246239e9360e8/oraclizeAPI_0.4.25.sol
    ...
    IERC721Enumerable
{
    ...
    /**
     * @dev Returns whBNBer `tokenId` exists.
     *
    ...
    /**
     * @dev Returns whBNBer `spender` is allowed to manage `tokenId`.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
     ...
     *
     * WARNING: Usage of this mBNBod is discouraged, use {_safeMint} whenever possible
     *
     * Requirements:
     *
     * - `tokenId` must not exist.
     ...
     * @return bool whBNBer the call correctly returned the expected magic value
     */
...
contract TestNFT is ERC721, Ownable {
    ...
    mapping(uint256 => bool) public currency;
    ...
    function mint(
        ...
        if (royalties > 0) {
            uint256 allowance =
                IBEP20(paymentTokenAddress).allowance(
                    msg.sender,
                    address(this)
                );
            uint256 token_bal =
                IBEP20(paymentTokenAddress).balanceOf(msg.sender); //how much MST buyer has
            require(allowance >= royalties, "Check the token allowance"); //is allowance provided
            require(token_bal >= royalties, "Error, the amount is lower");
            require(
                IBEP20(paymentTokenAddress).transferFrom(
                    msg.sender,
                    address(this),
                    royalties
                )
    ...
    function buy(uint256 _id) external payable {
        ...
        // 5% commission cut
        uint256 _commissionValue = price[_id] / 20;
    ...
    function _validate(uint256 _id) internal view {
        ...
        if (currency[_id]) {
            require(
                address(msg.sender).balance >= price[_id],
     ...
     */
    function setPaymentToken(address newBEP20Contract)
        external
        onlyOwner
        returns (bool)
    {
        paymentTokenAddress = newBEP20Contract;
    }
    ...
    function withdraw(address payable sendTo, uint256 amount)
        external
        onlyOwner
    {
    ...
    function withdrawTokens(address to_wallet, uint256 realAmountTokens)
        external
        onlyOwner
    {
        require(
            realAmountTokens > 0,
            "You need to withdraw at least some tokens"
        );
        uint256 contractTokenBalance =
            IBEP20(paymentTokenAddress).balanceOf(address(this));
        require(contractTokenBalance > realAmountTokens, "unsufficient funds");
        //ensure we revert in case of failure
        try IBEP20(paymentTokenAddress).transfer(to_wallet, realAmountTokens) {
